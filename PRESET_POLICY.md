...
1. **默认强制固定帧率**（2026-09-21 起，见下方"合并安全（CFR）策略"）：所有重新编码视频的预设加入 `-fps_mode cfr`。~~不强制固定帧率：移除 `-fps_mode cfr`~~ 是 2026-04-13 建立的原始策略，当时的理由是避免给正常可变帧率素材做不必要的补帧/丢帧、增大体积；2026-09-21 发现该策略会让源文件本身的丢帧/音画时长不一致在合并多段素材时不被纠正、反而逐段累积暴露，因此改为默认开启，纯 copy / 纯音频预设不适用。若确认某素材是合法可变帧率且不会被拿去和其他素材合并，可在预设副本里自行去掉该参数。
2. 不做音量放大：移除 `volume=...`。
...
## 合并安全（CFR）策略（2026-09-21 起，第六轮）

**触发原因**：用户用 `2压 H265 10bit NVENCfor720pXiaoMiTv` 下的预设把 4 段视频合并压制，发现末段音频和画面没对上。根因追溯到源文件（很可能是录屏时电脑在跑 ETH 挖矿、CPU/GPU 被抢占导致丢帧）内部音视频轨时长本就不一致，而当时的全局策略明确不强制 CFR，导致这种偏差在合并多段素材时未被纠正、逐段累积后在后面的片段变得可见。

**适用范围**：全仓库"会重新编码视频"的现役预设——即 `<encparamBox>` 里 `-c:v` 不是 `copy` 也不是纯音频输出的文件，覆盖 `libx265`/`hevc_nvenc`/`hevc_qsv`/`libx264`/`h264_nvenc` 五种编码器，共 337 个文件（含 `00_TEMPLATE_MASTER` 的 4 个母版、`舟 滤镜参考` 的 15 个文件；`SameAudio` 系列因为视频轨仍是重新编码，也在范围内）。

**不适用**：`0压 视频复制流`（纯 `-c:v copy`）、`0压 音频复制流` + `2压 音频预设`（纯音频输出，没有视频轨）、`舟 5.X原预设` / `舟 6.0原预设`（历史版本快照，不随现行规范变动）、`00_TEMPLATE_MASTER` 的两个纯滤镜片段（没有 `<encparamBox>`）。

**具体参数**：在 `<encparamBox>` 的 `-c:v` 那一组视频参数末尾追加 `-fps_mode cfr`，不改动其他任何参数（`qmin`/`qmax`/AQ/`preset`/CRF 等维持各文件夹既有差异化设置）。

**已知取舍**：对于真正按设计使用可变帧率、且确定不会被拿去和其他素材合并的源（比如某些游戏录制/自适应刷新率内容），`-fps_mode cfr` 可能导致补帧/丢帧、体积略增。这次决策认为"合并时不再音画错位"的正确性收益优先于这点体积代价；如果用户有明确不需要合并、且素材已知是合法 VFR 的场景，可以在预设副本里自行去掉这个参数。

依据、根因分析、执行方式详见 `design_documents/2026-09-21-merge-safety-fps-mode-cfr.md`。
