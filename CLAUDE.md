...
**2026-09-21 update (round 4)**: the `hevc_nvenc` folders below (`2压 H265 10bit NVENC`, `2压 H265 10bit NVENCfor720pXiaoMiTv`, `nvQuality`, `qualitySameAudio`, `qualitySameAudioTransTo1080p`, `qualityTransTo1080p`, `qualityTransTo720p`, plus `00_TEMPLATE_MASTER`'s `nvenc_cq23.xml`) got the NVENC-native translation of the same policy: true 10-bit via `-pix_fmt p010le` (not `yuv420p10le` — different from libx265), `-spatial-aq 1 -aq-strength 8` (NVENC's generic AQ, not an official anti-banding mode like x265's `aq-mode 3`), `-qmin 12`/no-qmax on standard CQ/VBR-bitrate rate-control modes only (not on fixed-QP or lossless variants, where qmin/qmax don't apply), no forced `-shanakeyframe`. `2压 H264 8bit NVENC` is explicitly excluded (stays 8-bit, untouched). `-pix_fmt p010le` has known driver/filter-chain compatibility risk — needs user testing. See NVENC policy in `PRESET_POLICY.md` and round 4 of `design_documents/2026-09-20-codexSlow-quality-tuning.md`.

**2026-09-21 update (round 5 — faststart audit)**: prompted by the v6.0.1.4→7.3.0.0 migration check, ran `/audit-presets` plus a full repo-wide sweep for `-movflags faststart` coverage. Fixed: 115 files across 10 `nvQuality`/`qualityQsv`/`quality*TransTo*`/`qualitySameAudio*` folders that were missing it despite producing MP4; `0压 视频复制流/复制为 MP4.xml`; `00_TEMPLATE_MASTER`'s `nvenc_cq23.xml`/`qsv_cq23.xml` masters; and 7 M4A-output files (`0压 音频复制流`'s M4A + 6 HE-AAC files in `2压 音频预设`) — `m4a` shares ffmpeg's mov/mp4 muxer, so faststart applies there too. Every active MP4/MOV/M4A-producing preset in the repo now has it. **Deliberately not applied** to MP3/OGG/AC3/FLAC/WAV presets (different muxer entirely — `-movflags` is a mov-muxer-private option, doesn't apply and isn't needed there) or to `舟 5.X原预设`/`舟 6.0原预设` (intentional historical snapshots of what those ShanaEncoder versions actually shipped — adding current conventions would falsify the record). Also fixed 4 unrelated consistency ERRORs (BigVoice SameAudio audio codec, a mislabeled NVENC CQ tier, two redundant duplicate files) and 2 WARNING-level audio-bitrate typos found along the way. Full writeup: `design_documents/2026-09-21-v7.3.0.0-migration-and-consistency-audit.md`.

**2026-09-21 update (round 6 — merge-safety CFR)**: triggered by a real user report — merging 4 clips with a `2压 H265 10bit NVENCfor720pXiaoMiTv` preset produced a final segment where the audio had already switched to the next source file while the picture was still showing the previous one. Root cause: one source recording (made while the machine was simultaneously mining ETH, starving the screen recorder of CPU/GPU) had a video track that dropped frames relative to its audio track, and the repo's then-current policy of never forcing CFR (`PRESET_POLICY.md`'s original global rule 1) meant that per-file drift was never corrected — it accumulated across concatenated segments and became visible by the last one. Fix: added `-fps_mode cfr` to the end of the `-c:v` line in every currently-active preset that actually re-encodes video (337 files: all `libx265`/`hevc_nvenc`/`hevc_qsv`/`libx264`/`h264_nvenc` folders, including the 4 `00_TEMPLATE_MASTER` masters and `舟 滤镜参考`'s 15 files — the latter treated as current/reference, not frozen, consistent with round 5's faststart treatment of it). **Not applied** to `0压 视频复制流` (`-c:v copy`, no re-encoding), `0压 音频复制流`/`2压 音频预设` (no video stream), or `舟 5.X原预设`/`舟 6.0原预设` (frozen historical snapshots, same rationale as round 5). This reverses the repo's prior "never force CFR" default; known trade-off is a possible size/frame-duplication cost for genuinely variable-frame-rate source material, accepted in favor of merge correctness. Full writeup: `design_documents/2026-09-21-merge-safety-fps-mode-cfr.md`.

| Folder | Codec | Bit | Quality | Preset | Audio | Notes |
|---|---|---|---|---|---|---|
| `00_TEMPLATE_MASTER` | (n/a) | — | — | — | — | Golden-master XML templates, NOT loaded directly in ShanaEncoder. Edit these first for global param changes, then propagate via generation script. Masters: `cpu_fast_crf21.xml` / `cpu_medium_crf21.xml` (libx265 fast/medium, CRF21, follow the unified CPU quantization policy above as of 2026-09-21), `nvenc_cq23.xml` (hevc_nvenc hq, CQ23, follows the unified NVENC quantization policy above as of 2026-09-21: `pix_fmt p010le`, `qmin 12`/no-qmax, `spatial-aq 1 -aq-strength 8`), `qsv_cq23.xml` (hevc_qsv veryfast, global_quality 23, qmin15/qmax35 — QSV, not subject to either AQ policy), `filter_scale720p.xml`/`filter_scale1080p.xml` (scale filters + scale_qsv alt, no full encparamBox). BigVoice = `-b:a 256k`, derived from base master (no separate master file). All 4 masters with a full `encparamBox` (i.e. not the two filter-only templates) now have `-movflags faststart` as of the round 5 audit. |
...
| `0压 音频复制流` | copy (audio) | — | lossless | — | copy | Audio-only extraction; M4A/MP3/OGG output. M4A file has `-movflags faststart` (mov-family muxer, benefits same as MP4); MP3/OGG intentionally don't (different muxer, not applicable). |
...
| `2压 H265 10bit NVENCfor720pXiaoMiTv` | hevc_nvenc | **true 10-bit** (`-pix_fmt p010le`) | Same tier/variant structure as `2压 H265 10bit NVENC` | hq | libmp3lame 128k, 2ch | faststart; scaled to 1280x720 w/ bicubic scale+pad for Xiaomi TV compatibility; same 2026-09-21 NVENC policy as above. |
...
| `2压 音频预设` | AC3 + other audio codecs | — | — | — | — | Audio-only, format/codec conversion. Output formats: AC3/FLAC/MP3/WAV (no faststart, different muxer — not applicable) and M4A/HE-AAC (6 files, all have `-movflags faststart` as of round 5, same mov-muxer rationale as MP4). |
...
| `nvQuality` | hevc_nvenc | **true 10-bit** (`-pix_fmt p010le`) | CQ 23–34 (per-file) | hq | libfdk_aac 192k | profile main10; unified NVENC quantization policy as of 2026-09-21 (`qmin 12`/no-qmax, `spatial-aq 1 -aq-strength 8`, no forced keyframe). |
...
| `qualitySameAudio` | hevc_nvenc | **true 10-bit** (`-pix_fmt p010le`) | CQ 23–33 (per-file) | hq | copy | profile main10 (NVENC despite generic folder name); unified NVENC quantization policy as of 2026-09-21 (see `nvQuality` row). |
| `qualitySameAudioTransTo1080p` | hevc_nvenc | **true 10-bit** (`-pix_fmt p010le`) | CQ 23–33 (per-file) | hq | copy | profile main10; scale/pad to 1920x1080; unified NVENC quantization policy as of 2026-09-21. |
| `qualityTransTo1080p` | hevc_nvenc | **true 10-bit** (`-pix_fmt p010le`) | CQ 23–33 (per-file) | hq | libfdk_aac 192k | profile main10; scale to 1920x1080 bicubic + pad; unified NVENC quantization policy as of 2026-09-21. |
| `qualityTransTo720p` | hevc_nvenc | **true 10-bit** (`-pix_fmt p010le`) | CQ 23–33 (per-file) | hq | libfdk_aac 192k | profile main10; scale to 1280x720; unified NVENC quantization policy as of 2026-09-21. |
